...
Један од кључних изазова у дистрибуираним архитектурама јесте чињеница да сервис може формално бити жив (енгл. _alive_), док у стварности није у стању да обрађује корисничке захтеве било због засићења ресурса, неконзистентног иницијалног стања или других облика делимичних отказа @tanenbaum2017 @kleppmann2017. Ова појава, позната као делимичан отказ (енгл. _partial failure_), представља један од најчешћих узрока нестабилности у дистрибуираним системима @nygard2018. Управо због тога постоји потреба за механизмима који непрекидно прате активност сервиса (енгл. _health-check_), детектују неправилности у раду система и омогућавају правовремене реакције система у циљу очувања стабилности.

Иако _Kubernetes_ поседује уграђене механизме за провере активности сервиса, они су доступни искључиво у _Kubernetes_ окружењима @kubernetes_probes и тешко их је прилагодити специфичним захтевима система. Чак и у случајевима када се _Kubernetes_ користи, ови механизми имају ограничења у погледу прилагодљивости логике провера, проширивости и интеграције са спољним системима @burns2018. Због тога постоји потреба за независним, конфигурабилним радним оквиром (енгл. _framework_) који омогућава сопствене типове провера здравља и већу контролу над понашањем система, без обзира на окружење у којем се апликације извршавају.

Потребно је направити систем који омогућава паралелно праћење стања више сервиса. Оно треба да обезбеди подршку различитих типова провера као што су покретање (енгл. _startup_), спремност (енгл. _readiness_) и активност (енгл. _liveness_), као и различитих комуникационих протокола, попут _HTTP_, _TCP_ и _gRPC_ @kubernetes_probes. Осим самог извођења провера, неопходно је обезбедити модел стања који на јасан начин описује тренутну исправност сервиса, као и механизме за аутоматизовано реаговање, попут поновног покретања контејнера, евидентирања догађаја и излагања метрика системима за надзор (енгл. _monitoring_). Централизована конфигурација помоћу _YAML_ формата додатно олакшава употребу система и његову интеграцију у различита окружења.

Циљ овог рада јесте развој самосталног радног оквира који омогућава унифицирано праћење исправности сервиса у дистрибуираним системима. Развијено решење подржава _startup_, _readiness_ и _liveness_ логике провера, ради са _HTTP_, _TCP_ и _gRPC_ протоколима, извршава провере конкурентно, излаже метрике у _Prometheus_ формату и омогућава конфигурацију преко _YAML_ датотека. Поред тога, радни оквир се интегрише са _Docker runtime_-ом ради аутоматизованих реакција на отказе сервиса.
...
Друго поглавље представља теоријске концепте неопходне за разумевање проблема и контекста у коме систем функционише. У њему су обрађени темељи контејнеризације, микросервисних архитектура, дистрибуираних система, механизама провере здравља и основа надгледања (енгл. _observability_), уз осврт на изазове који прате ове технологије.
